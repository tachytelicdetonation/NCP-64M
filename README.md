...
| `torch.compile`, B=48 | ~108k |
| `max-autotune-no-cudagraphs`, B=64 | ~119k |
| `max-autotune-no-cudagraphs`, B=96 | ~125k |

**~2.7× faster than the eager baseline** — compile is on by default on CUDA
(`--compile 0` to disable, `--compile_mode` to pick). Recommended:
`--batch_size 64 --compile_mode max-autotune-no-cudagraphs`. Avoid B≥96 — the
monitor's eager eval/diag probes need VRAM headroom.

- `torch.compile` fuses the routing/norm/RoPE elementwise chains and picks
  flash-attention kernels. Use the `*-no-cudagraphs` variant of max-autotune:
  inductor's cudagraph pools pin ~10GB and starve the monitor's probes.
...
- TF32 matmul enabled; `pin_memory` + `persistent_workers` on CUDA;
  ...
  5-D gather; eval probes cap batch at 32 to bound the eager spike.
- `--accumulation_steps` for larger effective batches. Note: eager IRC/CRC
  routing loops are *kept* — inductor fuses them better than a hand-rolled
  stack+einsum (measured regression).
